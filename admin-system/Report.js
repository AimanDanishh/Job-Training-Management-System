...
function getCanonicalTrainingStatus(t) {
  ...
  const appStatus = String(t.ApprovalStatus || '').trim().toLowerCase();
  const statusStr = String(t.Status || '').trim().toLowerCase();
  const stageStr = String(t.Stage || '').trim().toLowerCase();

  // 1. Explicitly Approved
  if (appStatus === 'approved' || appStatus === 'auto-approved' || appStatus === 'fully approved') {
    if (statusStr.includes('progress') || statusStr.includes('ongoing') || stageStr.includes('attendance')) return 'Ongoing';
    if (statusStr.includes('completed') || stageStr.includes('complete') || stageStr.includes('closed') || stageStr.includes('review')) return 'Completed';
    return 'Approved';
  }
  ...
  // 3. Returned
  if (appStatus.includes('return') || (statusStr.includes('return') && !appStatus.includes('pending') && !appStatus.includes('approved'))) {
    return 'Returned';
  }
  ...
  // 7. Fallback Approved
  if (appStatus.includes('acknowledged') || statusStr.includes('approved') || statusStr.includes('upcoming')) {
    return 'Approved';
  }

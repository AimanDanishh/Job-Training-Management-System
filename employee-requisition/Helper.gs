...
function createTrainingRequisitionForm(code, training, targetFolderId, requesterSigData) {
  ...
  try {
    ...
    const trList = parseTrainersData(tData.trainers || tData.Trainers, tData.Trainer);
    const trDisplay = trList.join(', ');
    let providerAndTrainer = '';
    const prov = tData.TrainingProvider || tData.Provider || '';
    if (prov && trDisplay) {
      providerAndTrainer = `${prov} (Trainer: ${trDisplay})`;
    } else {
      providerAndTrainer = prov || trDisplay || '';
    }
    ...
    populateRequisitionFormParticipantsAndTabs(ss, sheet, partList);
...
function syncTrainingRequisitionParticipants(trainingId, directParticipantsList) {
  try {
    ...
    populateRequisitionFormParticipantsAndTabs(formSpreadsheet, primarySheet, resolved.participants);
...
function ensureTrainingSheetColumns(sheet) {
  ...
  const requiredHeaders = [
    ...
    'ParticipantList', 'Trainers', 'Attachments'
  ];
